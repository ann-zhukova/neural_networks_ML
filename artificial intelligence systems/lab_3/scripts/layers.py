...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        #print(x.shape, x_hat.shape, gamma.shape, beta.shape)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for n in range(N):
        for f in range(F):
            ...
            for c in range(C):
                padded_x = np.pad(x[n,c],pad_width = pad,mode = 'constant',constant_values = 0)
...
def max_pool_forward_naive(x, pool_param):
    ...
    H_new = 1 + (H - pool_height) // pool_stride
    W_new = 1 + (W - pool_width) // pool_stride
